...
# Wires all infrastructure modules together for the prod environment.
# Uses production-grade instance sizes, HA NAT gateways, and immutable ECR tags.
...
provider "aws" {
  ...
  # Recommended: export AWS_PROFILE=your-prod-profile
  # Or use IAM instance profiles / EKS IRSA for CI/CD.
  # IMPORTANT: Use a restricted prod IAM role — not an admin account.
}
...
module "vpc" {
  ...
  # One NAT gateway per AZ for HA in production.
...
module "ecr" {
  ...
  environment          = var.environment
  # IMMUTABLE tags prevent overwriting — required for prod reproducibility.
